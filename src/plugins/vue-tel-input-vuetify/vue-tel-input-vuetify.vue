<template>
  <div :class="['vue-tel-input-vuetify', wrapperClasses, vertical ? '' : 'vertical d-flex']">
      ...
      >
        <template v-slot:selection>
          <div class="d-flex align-center">
            ...
            <div>{{ `+${activeCountry.dialCode}` }}</div>
            <v-icon class="ml-n1 mr-3">mdi-menu-down</v-icon>
            <div v-if="vertical">{{ activeCountry.name }}</div>
          </div>
        </template>
...
<script>
  ...
  import utils, { getCountry, setCaretPosition } from './lib/utils'
  ...
  export default {
    ...
    mounted() {
      ...
      this.initializeCountry()
        .then(() => {
          if (!this.phone && this.inputOptions && this.inputOptions.showMask && this.activeCountry) {
            this.placeholder = PhoneNumber.getExample(this.activeCountry.iso2).getNumber('national')
          }
          this.countryCode = this.activeCountry
          if (this.phone) this.phone = this.phoneObject.number.national
          this.$emit('validate', this.phoneObject)
          this.$emit('onValidate', this.phoneObject) // Deprecated
        })
        .catch(console.error)
    ...
    methods: {
      ...
      choose(country, toEmitInputEvent = false) {
        ...
        if (this.phone && this.phone[0] === '+' && this.activeCountry.iso2 && this.phoneObject.number.significant) {
          ...
          this.phone = PhoneNumber(this.phoneObject.number.significant, this.activeCountry.iso2).getNumber('national')
        }
        if (this.inputOptions && this.inputOptions.showMask && country) {
          // Reset phone if the showMask is set
          this.placeholder = PhoneNumber.getExample(this.activeCountry.iso2 || this.activeCountry).getNumber('national')
        }
        if (toEmitInputEvent) {
          this.$emit('input', this.phoneText, this.phoneObject)
      ...
      onInput(e) {
        ...
        this.$nextTick(() => {
          var ayt = PhoneNumber.getAsYouType(this.phoneObject.regionCode)
          this.phone = ayt.reset(this.phoneText)
        })
      },
      onBlur() {
        this.$emit('blur')
...
<style src="./lib/sprite.css"></style>
<style lang="scss">
  .vti__flag {
    margin-right: 2px;
    margin-top: -3px;
      ...
      & > .v-input {
        .v-text-field__slot {
          ...
          label {
            left: 70px !important;
            &.v-label--active {
              transform: scale(0.85) translateY(-26px) translateX(-81px);
            }
          }
        }
      }
