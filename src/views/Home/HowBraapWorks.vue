<template>
  <v-navigation-drawer
    ...
    fixed
    temporary
    class="rounded-0 rounded-tl-10 rounded-bl-10"
    overlay-color="rgba(0,0,0,0.5)"
    overlay-opacity="1"
    width="70%"
  >
    <div class="how-braap-works">
      <v-btn color="primary" large class="all-blog-btn rounded-10 body-0 px-6 text-capitalize" dark>
        <v-icon class="mt-1 mr-1 relative mr-1" color="white">$svg-arrow-left</v-icon>
        <span class="text-subtitle-1">Back</span>
      </v-btn>

      <div class="how-to-work braap-content">
        <div class="sub-title primary--text relative font-weight-bold">
          How
          <span class="braap_orange--text">Braap</span>
          Works
        </div>
        <p class="mt-10 mb-0">
          Mechanical parts break, when things go wrong they need fixed. Long before you need a diagnosis for your
          problem Braap is connecting the millions of parts, maintenance requirements, and service professionals so you
          can easily get what you need.
        </p>
        <div class="mt-10">
          <BraapSearch class="header-search" :readonly="true" :placeholder="text"></BraapSearch>
        </div>
      </div>

      <v-divider></v-divider>

      <div class="organizing-and-connecting braap-content d-flex">
        <div class="flex-left">
          <div class="sub-title primary--text relative font-weight-bold">
            Organizing
            <span class="braap_orange--text">and</span>
            Connecting
          </div>
          <p class="mb-0 mt-10">
            Braap connects diagnostics, know-how, parts, and service technicians together on one platform to solve
            problems. Our AI engine helps keep track of maintenance needs, fixes, and everything else to make your
            mechanical device run its best.
          </p>
        </div>

        <div class="flex-right d-flex justify-center align-center">
          <svg
      ...
      <div class="instant-results braap-content d-flex">
        <div class="flex-left">
          <div class="sub-title primary--text relative font-weight-bold">
            Instant
            <span class="braap_orange--text">Results</span>
          </div>
          <p class="mb-0 mt-10">
            In a fraction of seconds, Braap uses its algorithms and ranking system to sort through millions of potential
            diagnostics and parts to find the most relevant result for what you are looking for.
          </p>
          <v-btn
            color="braap_orange"
            x-large
            dark
            class="all-features-btn rounded-10 px-8 text-capitalize body-0 mt-10"
          >
            <span>Ask Technician for Help</span><v-icon class="ml-2">mdi-arrow-right</v-icon>
          </v-btn>
        </div>

        <div class="flex-right">
          <img src="/img/home/instant-results.jpg" />
        </div>
      </div>
      ...
      <div class="get-help">
        <div class="sub-title primary--text relative font-weight-bold">
          Get help
          <span class="braap_orange--text">or just</span>
          get what you need
        </div>
        <p class="mb-0 mt-10">
          Braap's innovative technology is a game-changer for the service industry. Tools that let you visually and
          acoustically diagnose a problem without time-consuming guesswork, simplifying the job.
        </p>
        <div class="mt-15 d-flex justify-center">
          <div class="get-help-search flex-left">
            <div class="d-flex justify-space-between">
              <img src="/img/home/camera-left.svg" />
              <div class="d-flex flex-column align-end">
                <img src="/img/home/sound.gif" class="img-gif mr-2 mb-2" />
                <img src="/img/home/mike-right.svg" />
              </div>
            </div>
            <BraapSearch class="get-help-search mt-5"></BraapSearch>
          </div>
        </div>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    data() {
      return {
        drawer: false,
        words: ['I hear a high pitched clicking sound.', 'My Engine does not start', 'Brakes are old and grinding'],
        word: null,
        index: 0,
        timer: null,
        text: ''
    ...
    methods: {
      typingEffect() {
        this.word = this.words[this.index].split('')
        this.loopTyping()
      },
      deletingEffect() {
        this.word = this.words[this.index].split('')
      ...
      loopDeleting() {
        ...
        this.timer = setTimeout(this.loopDeleting, 30)
      },
      loopTyping() {
        if (this.word.length > 0) {
        ...
        this.timer = setTimeout(this.loopTyping, 100)
      }
    },
    mounted() {
      this.typingEffect()
...
<style lang="scss">
  .how-braap-works {
    ...
    #whileB {
      ...
      animation: whileRotate 0.8s infinite linear;
    }
    @keyframes whileRotate {
      0% {
        transform: rotate(0deg);
      }
