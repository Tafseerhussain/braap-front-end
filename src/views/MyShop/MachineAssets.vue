<template>
  <div class="machine-assets">
    <v-container class="py-0">
      <v-row>
        <v-col cols="6">
          <v-btn color="braap_orange" dark x-large class="mr-4 mb-0 rounded-10" @click="addMachine">
            + Add Machine | Asset
          </v-btn>
        </v-col>
        <v-col cols="6">
...
<script>
  ...
  export default {
    name: 'MachineAssets',
    components: { MachineAsset, MachineAssetSkeleton, AddMachineDialog },
    props: {
      value: {
        type: String
      },
    ...
    methods: {
      ...
      restoreMachine(machine) {
        this.$axios.post('/api/vehicle/revert/' + machine.id).then(result => {
          if (result.success && result.data) {
            this.$msg.success('The machine is in your shop, you can access it now.')
      ...
      removeMachine(machine) {
        this.$axios.post('/api/vehicle/delete/' + machine.id).then(result => {
          if (result.success && result.data) {
            if (!result.data) {
              this.$msg.warning('Machine can not be removed it has a pending job, aintenance, or part on order.')
            } else {
              this.$msg.success('The machine is in the recycling bin in case you want to access it later.')
              var index = this.machines.findIndex(x => x.id === machine.id)
              this.machines.splice(index, 1)
