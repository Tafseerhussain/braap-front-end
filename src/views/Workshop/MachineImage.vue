...
<script>
  import MachineImageMarker from './MachineImageMarker.vue'
  ...
  export default {
    ...
    methods: {
      ...
      getMarkers() {
        this.$axios.post(`/api/vehicle/${this.vehicleId}/images/${this.imageId}/markers/list`).then(result => {
      ...
      getMarkerSystems() {
        this.$axios.post(`/api/vehicle/${this.vehicleId}/images/${this.imageId}/markers/system`).then(result => {
          if (result.success && result.data) {
      ...
      onMarkerRemoved({ id, vehicleWorkPartId }) {
        var index = this.markers.findIndex(m => m.id === id)
        if (index >= 0) {
          this.markers.splice(index, 1)
          this.$emit('update:count', this.markers.length)
          this.$emit('marker-removed', vehicleWorkPartId)
        }
      }
    }
  }
